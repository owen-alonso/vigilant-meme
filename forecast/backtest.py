...
def quantile_weights(
    ...
    *,
    ...
    """Dollar-neutral long top / short bottom. Two names: +0.5 / -0.5 on the spread."""
    s = scores.dropna()
...
def book_pnl(
    ...
    *,
    ...
    """Vol-scaled quantile long-short. Costs from one-way turnover * half round-trip."""
    dates = pred.index.intersection(realized.index)
    weights: list[pd.Series] = []
    gross: list[float] = []
    for ts in dates:
        w = quantile_weights(pred.loc[ts], quantile=quantile)
        r = realized.loc[ts].reindex(w.index)
