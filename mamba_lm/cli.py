"""CLI: python -m mamba_lm.cli train | benchmark | compare | report."""
...
def _device(args: argparse.Namespace) -> torch.device:
    return torch.device("cpu") if getattr(args, "cpu", False) else select_device()
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.cmd == "train":
        train(_model_cfg(args), _train_cfg(args), device=_device(args))
        return
