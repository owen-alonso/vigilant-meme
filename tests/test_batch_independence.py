...
from tests.helpers import block_scale, dynamic_config, perturb_controller
...
def test_batch_independence_of_dynamic_A():
    ...
    with torch.no_grad():
        block(x)
        scale_orig = block_scale(block).detach().clone()

    x_mut = x.clone()
    x_mut[0] = torch.randn_like(x_mut[0])
    with torch.no_grad():
        block(x_mut)
        scale_mut = block_scale(block).detach().clone()
